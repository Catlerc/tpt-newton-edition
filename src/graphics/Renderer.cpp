...
void Renderer::render_gravlensing(const RendererFrame &source) {
    for (auto p: RES.OriginRect()) {
        auto cp = p / CELL;
        auto rp = Vec2{
            int(p.X - sim->gravOut.forceX[cp] * 0.75f + 0.5f), int(p.Y - sim->gravOut.forceY[cp] * 0.75f + 0.5f)
        };
        auto gp = Vec2{
            int(p.X - sim->gravOut.forceX[cp] * 0.875f + 0.5f), int(p.Y - sim->gravOut.forceY[cp] * 0.875f + 0.5f)
        };
    ...
    for (i = 0; i <= sim->parts.lastActiveIndex; i++) {
        if (sim->parts[i].type && sim->parts[i].type >= 0 && sim->parts[i].type < PT_NUM) {
            ...
            if (nx >= XRES || nx < 0 || ny >= YRES || ny < 0)
                continue;
            if (TYP(sim->photons[ny][nx]) && !(elements[t].Properties & TYPE_ENERGY) && t != PT_STKM && t != PT_STKM2 &&
                t != PT_FIGH)
                continue;
            ...
            if (decorationLevel == decorationAntiClickbait) {
                ...
                if (graphicscache[t].isready) {
                    ...
                    auto makeReady = !graphics || graphics(gfctx, &(sim->parts[i]), nx, ny, &pixel_mode, &cola, &colr,
                                                           &colg, &colb, &firea, &firer, &fireg,
                                                           &fireb); //That's a lot of args, a struct might be better
                ...
                if ((elements[t].Properties & PROP_HOT_GLOW) && sim->parts[i].temp > (
                        elements[t].HighTemperature - 800.0f)) {
                    auto gradv = 3.1415 / (2 * elements[t].HighTemperature - (elements[t].HighTemperature - 800.0f));
                    auto caddress = int((sim->parts[i].temp > elements[t].HighTemperature)
                                            ? elements[t].HighTemperature - (elements[t].HighTemperature - 800.0f)
                                            : sim->parts[i].temp - (elements[t].HighTemperature - 800.0f));
                ...
                if (colorMode & COLOUR_HEAT) {
                    firea = 255;
                    RGB color = heatTableAt(int(
                        (sim->parts[i].temp - stats.hdispLimitMin) / (stats.hdispLimitMax - stats.hdispLimitMin) *
                        1024));
                    ...
                    if (findingElement->property.Offset == offsetof(Particle, type)) {
                        ...
                        switch (findingElement->property.Type) {
                            case StructProperty::Float:
                                matchesFindingElement =
                                        *((float *) (((char *) &sim->parts[i]) + findingElement->property.Offset)) ==
                                        std::get<float>(findingElement->value);
                                break;

                            case StructProperty::ParticleType:
                            case StructProperty::Integer:
                                matchesFindingElement =
                                        *((int *) (((char *) &sim->parts[i]) + findingElement->property.Offset)) ==
                                        std::get<int>(findingElement->value);
                                break;

                            case StructProperty::UInteger:
                                matchesFindingElement =
                                        *((unsigned int *) (
                                            ((char *) &sim->parts[i]) + findingElement->property.Offset)) == std::get<
                                            unsigned int>(findingElement->value);
                                break;
                ...
                if (pixel_mode & EFFECT_LINES) {
                    if (t == PT_SOAP) {
                        if ((parts[i].ctype & 3) == 3 && parts[i].tmp >= 0 && parts[i].tmp < NPART)
                            BlendLine({nx, ny}, {int(parts[parts[i].tmp].x + 0.5f), int(parts[parts[i].tmp].y + 0.5f)},
                                      RGBA(colr, colg, colb, cola));
                    }
                }
                if (pixel_mode & PSPEC_STICKMAN) {
                    ...
                    if (mousePos.X > (nx - 3) && mousePos.X < (nx + 3) && mousePos.Y < (ny + 3) && mousePos.Y > (
                            ny - 3)) //If mouse is in the head
                    {
                        String hp = String::Build(Format::Width(sim->parts[i].life, 3));
                        BlendText(mousePos + Vec2{
                                      -8 - 2 * (sim->parts[i].life < 100) - 2 * (sim->parts[i].life < 10), -12
                                  }, hp, 0xFFFFFF_rgb .WithAlpha(255));
                    }
                    ...
                    DrawLine({int(cplayer->legs[0]), int(cplayer->legs[1])},
                             {int(cplayer->legs[4]), int(cplayer->legs[5])}, RGB(legr, legg, legb));
                    DrawLine({nx, ny + 3}, {int(cplayer->legs[8]), int(cplayer->legs[9])}, RGB(legr, legg, legb));
                    DrawLine({int(cplayer->legs[8]), int(cplayer->legs[9])},
                             {int(cplayer->legs[12]), int(cplayer->legs[13])}, RGB(legr, legg, legb));
                    if (cplayer->rocketBoots) {
                        for (int leg = 0; leg < 2; leg++) {
                            int nx = int(cplayer->legs[leg * 8 + 4]), ny = int(cplayer->legs[leg * 8 + 5]);
                            int colr = 255, colg = 0, colb = 255;
                            if (((int) (cplayer->comm) & 0x04) == 0x04 || (
                                    ((int) (cplayer->comm) & 0x01) == 0x01 && leg == 0) || (
                                    ((int) (cplayer->comm) & 0x02) == 0x02 && leg == 1))
                ...
                if (pixel_mode & EFFECT_GRAVIN) {
                    ...
                    for (r = 0; r < 4; r++) {
                        ...
                        if (ny + nyo > 0 && ny + nyo < YRES && nx + nxo > 0 && nx + nxo < XRES && TYP(
                                sim->pmap[ny + nyo][nx + nxo]) != PT_PRTI)
                ...
                if (pixel_mode & EFFECT_GRAVOUT) {
                    ...
                    for (r = 0; r < 4; r++) {
                        ...
                        nyo = (int) (ddist * sin(drad));
                        if (ny + nyo > 0 && ny + nyo < YRES && nx + nxo > 0 && nx + nxo < XRES && TYP(
                                sim->pmap[ny + nyo][nx + nxo]) != PT_PRTO)
                            AddPixel({nx + nxo, ny + nyo}, RGBA(colr, colg, colb, 255 - orbd[r]));
                    }
                }
