...
std::unique_ptr<VideoBuffer> GlueTool::GetIcon(int toolID, Vec2<int> size)
{
...
void GlueTool::Click(Simulation * sim, Brush const &brush, ui::Point position)
{
	auto points = sim->physicsSimulation.MakeHull(position.X, position.Y);
}
